...
> 반복 작업 시 이 테이블만 참조. 상세 가이드는 아래 Step별 섹션 참고.

| Figma 속성 | Atomic 패턴 | 예시 |
|-----------|------------|------|
| fill color | `bg{HEX}` | `bg0F172A` |
| text color | `c{HEX}` | `cFFFFFF` |
| rgba fill | `bg{R}-{G}-{B}-{A}` | `bg0-0-0-60` |
| fontSize | `fs{값}px` | `fs15px` |
| fontWeight | `fw{값}` | `fw700` |
| lineHeight | `lh{값}px` | `lh25px` |
| padding | `p{값}px` / `p{rem}` | `p2rem` (20px+) |
| gap | `gap{값}px` | `gap12px` |
| border | `b1pxsolid{HEX}` | `b1pxsolid2E3647` |
| border-radius | `br{값}px` | `br12px` |
| display flex | `df` | `df aic gap8px` |
| display grid | `dg` | `dg gtc1fr-1fr` |

**간격 스냅:** 4의 배수, 20px 이상 → rem (1rem=10px)
**색상:** Figma HEX 그대로, 대문자 6자리
**모를 때:** 반드시 MCP 조회 (`validate_classes`, `search_by_css`)
...
### Figma MCP가 React+Tailwind 코드를 반환하는 경우 (우선 경로)

Figma MCP(`get_design_context` 등)는 보통 **React JSX + Tailwind CSS 클래스** 형태로 코드를 반환합니다.

1. **Tailwind 클래스에서 CSS 값 추출** — Tailwind 클래스를 CSS 선언으로 역변환
   - `bg-[#0f172a]` → `background-color: #0f172a`
   - `border-[var(--color/line,-btn,#2e3647)]` → fallback값 `#2e3647` 추출 → `border-color: #2e3647`
   - `px-[var(--20,20px)]` → fallback값 추출 → `padding-left: 20px; padding-right: 20px`
   - `gap-[var(--8,8px)]` → `gap: 8px`
   - `text-[15px]` → `font-size: 15px`
   - `leading-[25px]` → `line-height: 25px`
   - `rounded-[var(--12,12px)]` → `border-radius: 12px`

2. **CSS 변수 fallback 파싱 규칙:**
   - `var(--name, fallback)` 형태에서 **fallback 값만 사용**
   - `var(--color\/line\,-btn,#2e3647)` → 이스케이프 제거 → fallback = `#2e3647`
   - `rgba(5,117,230,0.2)` → 그대로 사용

3. **추출한 CSS를 `css_to_classes`로 일괄 변환**
4. **React JSX → HTML 변환** — `className` → `class`, JSX 셀프클로징 → HTML 태그, 이벤트 핸들러 제거
5. **`validate_classes`로 최종 검증**

### Tailwind → Atomic 즉석 변환 치트시트

Figma MCP 출력을 보면서 기계적으로 Atomic 클래스를 뽑아내는 테이블:

| Figma MCP (Tailwind) | 추출 CSS | Atomic CSS |
|---------------------|----------|------------|
| `bg-[var(--x,#0f172a)]` | background: #0F172A | `bg0F172A` |
| `bg-[var(--x,rgba(0,0,0,0.6))]` | background: rgba(0,0,0,0.6) | `bg0-0-0-60` |
| `border border-[var(--x,#2e3647)] border-solid` | border: 1px solid #2E3647 | `b1pxsolid2E3647` |
| `rounded-[var(--12,12px)]` | border-radius: 12px | `br12px` |
| `p-[var(--20,20px)]` | padding: 20px | `p2rem` |
| `px-[var(--16,16px)]` | padding-left/right: 16px | `px16px` |
| `gap-[var(--8,8px)]` | gap: 8px | `gap8px` |
| `flex flex-col` | display:flex; flex-direction:column | `df fdc` |
| `items-center justify-between` | align-items:center; justify-content:space-between | `aic jcsb` |
| `text-[15px]` | font-size: 15px | `fs15px` |
| `font-['Pretendard:Bold']` | font-weight: 700 | `fw700` |
| `font-['Pretendard:Medium']` | font-weight: 500 | `fw500` |
| `leading-[25px]` | line-height: 25px | `lh25px` |
| `text-[color:var(--x,#9ca3af)]` | color: #9CA3AF | `c9CA3AF` |
| `text-white` | color: #fff | `cFFFFFF` |
| `whitespace-nowrap` | white-space: nowrap | `wsn` |
| `overflow-clip` | overflow: hidden | `oh` |
| `shrink-0` | flex-shrink: 0 | `fs0` |
| `flex-[1_0_0]` | flex: 1 0 0 | `fg1` |
| `h-[48px]` | height: 48px | `h48px` |
| `w-full` | width: 100% | `w100p` |
| `size-[52px]` | width:52px; height:52px | `w52px h52px` |

> **치트시트에 없는 클래스는 반드시 `css_to_classes` 또는 `search_by_css`로 조회할 것.**
...
### Figma MCP 에셋 URL의 한계

Figma MCP가 반환하는 `https://www.figma.com/api/mcp/asset/...` URL은 **7일 만료 임시 URL**입니다.
코드에 직접 삽입하면 안 됩니다.

**아이콘 처리 우선순위:**
1. **기존 프로젝트에 같은 아이콘이 있으면** → 기존 SVG 재사용 (grep으로 검색)
2. **기존에 없으면** → Figma 에셋 URL을 브라우저에서 열어 SVG 코드를 복사하거나, Figma 디자인 데이터의 `data-name` 속성을 참고하여 의미에 맞는 SVG를 직접 작성
3. **복잡한 아이콘이라 직접 작성이 어려우면** → 사용자에게 Figma에서 SVG export 요청

**절대 금지:** Figma 에셋 URL을 `<img src="...">` 로 코드에 삽입하는 것
...
기존 프로젝트의 글로벌 스타일시트(reset, common, normalize 등)가 요소 셀렉터로 input/select/textarea 등에 스타일을 적용하고 있을 수 있습니다.

**흔한 충돌 패턴:**
| 글로벌 셀렉터 | Specificity | 컴포넌트 클래스 | Specificity | 결과 |
|--------------|-------------|----------------|-------------|------|
| `input[type="number"]` | 0,1,1 | `.field` | 0,1,0 | 글로벌 스타일 승 |
| `select` | 0,0,1 | `.field` | 0,1,0 | 클래스 승 |
| `input[type="text"]:focus` | 0,2,1 | `.field:focus` | 0,2,0 | 글로벌 스타일 승 |

**적용 전 확인:**
1. 해당 요소의 글로벌 스타일을 DevTools에서 확인
2. 컴포넌트 클래스가 글로벌보다 specificity가 높은지 확인
3. 낮으면 `input.field`, `select.field` 등으로 요소 셀렉터를 결합하여 specificity를 올림

**원칙:** `!important`는 사용하지 않는다. 셀렉터 결합으로 해결한다.
